...
function check_command(audio, type){
    ...
    else if (audio.includes("calendar")){
        number = null
        if(audio.includes("set")){
            open_calendar()

            if (hasNumber(audio)){
                number = audio.match(/\d+/)[0]
            }
            setTimeout(() => {  get_calendar_info(number) }, 3000);
        }else{
            open_calendar()
        }
...
function setTimer(countDownDate){
    ...
    if(countDownDate == "reset"){
        clearInterval(timerInterval);
        speak("timer resetted")
        countDownDate = "timer";
    }

    if(countDownDate == "timer"){
        ...
        timerInterval = setInterval(function() {
           ...
           if ( sec < 59){
            ...
            console.log(min, sec);
            if( hour >= 1){
                (hour < 10) ? $("#timer-1").html("0" + hour ) : $("#timer-1").html( hour );
                (min < 10) ? $("#timer-2").html("0" + min ) : $("#timer-2").html( min );
            }else{
                (min < 10) ? $("#timer-1").html("0" + min ) : $("#timer-1").html( min );
                (sec < 10) ? $("#timer-2").html("0" + sec ) : $("#timer-2").html( sec );
            }
