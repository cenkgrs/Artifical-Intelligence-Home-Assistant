...
#             os.system('play -nq -t alsa synth {} sine {}'.format(0.15, 500))  # 1 = duration, 440 = frequency -> this sounds a beep
    ...
    conn.close()
def check_alarm():
    cn = sqlite3.connect('/home/pi/Oracle/Oracle')
    cs = cn.cursor()
    check_hour = datetime.datetime.now().hour
    check_min = datetime.datetime.now().minute

    try:
        db = cs.execute('SELECT * FROM Alarms ORDER BY hour ASC')
        last = db.fetchmany()
        print(last)
        ...
        if check_hour == last[0][1] and check_min == last[0][2]:
            ...
            cs.execute("DELETE FROM Alarms WHERE hour = ? and minute = ?", (check_hour, check_min))
            cn.commit()
